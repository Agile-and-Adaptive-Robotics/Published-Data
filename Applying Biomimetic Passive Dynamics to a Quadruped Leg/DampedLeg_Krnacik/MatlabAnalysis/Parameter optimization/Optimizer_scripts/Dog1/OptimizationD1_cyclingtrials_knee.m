...
% folder_title = 'AllTrialResultsD1_knee';
...
% Set initial conditions for the knee from that respective trial
b_i = [dynchk_results.b_i(2)];
k_i = [dynchk_results.k_i(2)];
IC_data = [b_i k_i];
U = IC_data;
...
k_bc = [ scalarup_k .* k_i; k_i ./ scalardwn_k ];
    
UB = [ b_bc(1, :) k_bc(1, :) ];
LB = [ b_bc(2, :) k_bc(2, :) ];

% Define parameters of hip and ankle to be set
hip_var = [dynchk_results.b_i(1) dynchk_results.k_i(1)];
